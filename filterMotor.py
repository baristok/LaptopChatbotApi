...
def filter_products(df: pd.DataFrame, filters: dict) -> pd.DataFrame:
    ...
    if 'max_price' in filters or 'min_price' in filters:
        filtered_df['Fiyat'] = filtered_df['Fiyat'].astype(str).str.replace(r'[^\d]', '', regex=True)
    ...
    # 🏷 Marka filtresi
    if 'brand' in filters:
        filtered_df = filtered_df[filtered_df['Marka'].str.lower() == filters['brand'].lower()]

    # 🧠 RAM
    if 'exact_ram' in filters:
